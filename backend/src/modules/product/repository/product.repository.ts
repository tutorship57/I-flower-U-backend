...
const productRepository = {
    ...
    decreaseStockTransaction: async (items: { product_id: string; quantity: number }[]) => {
    return await prisma.$transaction(async (tx) => {
      for (const item of items) {
        const p = await tx.product.findUnique({ where: { product_id: item.product_id } });
        if (!p) throw new Error(`Product ${item.product_id} not found`);
        if (p.product_stock < item.quantity) throw new Error(`Not enough stock for ${item.product_id}`);
    ...
    increaseStockTransaction: async (items: { product_id: string; quantity: number }[]) => {
    return await prisma.$transaction(async (tx) => {
      for (const item of items) {
        await tx.product.update({
          where: { product_id: item.product_id },
